...
import { ClinicDetail } from 'src/app/models/dtos/clinicDetail';
...
export class ColorsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.getCurrentUser();
    this.getClinicDetail(this.currentUser.veterinaryClinicId);
  }
  ...
  getClinicDetail(clinicId: number) {
    this.vetClinicService.getClinicDetails(clinicId).subscribe(response => {
